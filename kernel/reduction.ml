...
let rec ccnv cv_pb l2r env infos lft1 lft2 term1 term2 cuniv =
  try
  ...
  with NotConvertible as ex ->
	...
	(inject ty) in
      ...
      | Cast(_,_,ty) -> aux ty
      | Prod (_,a,b) -> aux b
      | Ext(e,_) when Extensions.equal (Extensions.iprf) e ->
	empty_constraint
      | _ -> raise (Errors.push ex)
    in
    let term1_constr = Vars.exliftn lft1 (whd_val (snd infos) term1) in
    try
    let the_type = (!Closure.nasty_infer env term1_constr) in
    aux the_type
    with _ -> raise (Errors.push ex)

(* Conversion between [lft1](hd1 v1) and [lft2](hd2 v2) *)
and eqappr cv_pb l2r env infos (lft1,st1) (lft2,st2) cuniv =
  ...
  match (fterm_of hd1, fterm_of hd2) with
	...
	(match kind_of_term a1, kind_of_term a2 with
	   ...
	   | (Meta n, Meta m) ->
               if Int.equal n m
	       then convert_stacks l2r env infos lft1 lft2 v1 v2 cuniv
               else raise NotConvertible
	   | _ -> raise NotConvertible)
    | (FEvar ((ev1,args1),env1), FEvar ((ev2,args2),env2)) ->
        if Evar.equal ev1 ev2 then
          let u1 = convert_stacks l2r env infos lft1 lft2 v1 v2 cuniv in
          convert_vect l2r env infos el1 el2
    ...
    | (FExt (e1,args1), FExt (e2,args2)) ->
        if Extensions.equal e1 e2 then
          let u1 = convert_stacks l2r env infos lft1 lft2 v1 v2 cuniv in
          convert_vect l2r env infos el1 el2 args1 args2 u1
        else raise NotConvertible
	...
	(try (* try first intensional equality *)
	  if eq_table_key fl1 fl2
        ...
        with NotConvertible ->
          ...
          eqappr cv_pb l2r env infos app1 app2 cuniv)
    ...
    | (FLambda _, FLambda _) ->
	  ...
	  anomaly (Pp.str "conversion was given ill-typed terms (FLambda)");
        let (na1,ty1,bd1) = destFLambda mk_clos hd1 in
        let (_,ty2,bd2) = destFLambda mk_clos hd2 in
        let u1 = ccnv CONV l2r env infos el1 el2 ty1 ty2 cuniv in
        ccnv CONV l2r (Environ.push_rel (na1,None,whd_val (snd infos) ty1) env)
	  infos (el_lift el1) (el_lift el2) bd1 bd2 u1

    | (FProd (n1,c1,c2), FProd (_,c'1,c'2)) ->
        if not (is_empty_stack v1 && is_empty_stack v2) then
	  anomaly (Pp.str "conversion was given ill-typed terms (FProd)");
	(* Luo's system *)
        let u1 = ccnv CONV l2r env infos el1 el2 c1 c'1 cuniv in
        ccnv cv_pb l2r (Environ.push_rel (n1,None,whd_val (snd infos) c1) env)
	  infos (el_lift el1) (el_lift el2) c2 c'2 u1
    ...
    | (FLambda _, _) ->
        ...
        in
        let (na1,ty1,bd1) = destFLambda mk_clos hd1 in
	eqappr CONV l2r (Environ.push_rel (na1,None,whd_val (snd infos) ty1) env) infos
    ...
    | (_, FLambda _) ->
	  ...
	  anomaly (Pp.str "conversion was given unreduced term (FLambda)")
	in
        let (na2,ty2,bd2) = destFLambda mk_clos hd2 in
	eqappr CONV l2r (Environ.push_rel (na2,None,whd_val (snd infos) ty2) env) infos
    ...
    | (FFlex fl1, _)      ->
        (match unfold_reference infos fl1 with
           | Some def1 ->
	       eqappr cv_pb l2r env infos (lft1, whd_stack (snd infos) def1 v1) appr2 cuniv
           | None -> raise NotConvertible)
    | (_, FFlex fl2)      ->
        (match unfold_reference infos fl2 with
           | Some def2 ->
	       eqappr cv_pb l2r env infos appr1 (lft2, whd_stack (snd infos) def2 v2) cuniv
           | None -> raise NotConvertible)
    ...
    | (FInd ind1, FInd ind2) ->
        if eq_ind ind1 ind2
	then
          convert_stacks l2r env infos lft1 lft2 v1 v2 cuniv
        else raise NotConvertible
	...
	then
	  ...
	  let u1 = convert_vect l2r env infos el1 el2 fty1 fty2 cuniv in
          let u2 =
            convert_vect l2r (Environ.push_rec_types
				(na1,tys1,cl1) env) infos
	      (el_liftn n el1) (el_liftn n el2) fcl1 fcl2 u1 in
          convert_stacks l2r env infos lft1 lft2 v1 v2 u2
        else raise NotConvertible
...
and convert_stacks l2r env infos lft1 lft2 stk1 stk2 cuniv =
  compare_stacks
    (fun (l1,t1) (l2,t2) c -> ccnv CONV l2r env infos l1 l2 t1 t2 c)
    (eq_ind)
    lft1 stk1 lft2 stk2 cuniv

and convert_vect l2r env infos lft1 lft2 v1 v2 cuniv =
  ...
  then
    let rec fold n univ =
      if n >= lv1 then univ
        ...
        let u1 = ccnv CONV l2r env infos lft1 lft2 v1.(n) v2.(n) univ in
        fold (n+1) u1 in
    fold 0 cuniv
  else raise NotConvertible
...
let clos_fconv trans cv_pb l2r evars env t1 t2 =
  ...
  ccnv cv_pb l2r env infos el_id el_id (inject t1) (inject t2) empty_constraint
