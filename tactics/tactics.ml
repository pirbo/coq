...
let symmetry_red allowred =
  ...
  | Some eq_data,_,_ ->
      Tacticals.New.tclTHEN
        ...
        (Tacticals.New.pf_constr_of_global eq_data.eq_data.sym apply)
...
let transitivity_red allowred t =
  ...
  | Some eq_data,_,_ ->
      Tacticals.New.tclTHEN
        (convert_concl_no_check concl DEFAULTcast)
        (match t with
	  | None -> Tacticals.New.pf_constr_of_global eq_data.eq_data.trans eapply
	  | Some t -> Tacticals.New.pf_constr_of_global eq_data.eq_data.trans (fun trans -> apply_list [trans;t]))
   | None,eq,eq_kind ->
      match t with
